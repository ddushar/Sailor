...
async def safe_followup_send(
    ...
    try:
        ...
        # 50027 = interaction token expired (Invalid Webhook Token).
    ...
    is_ephemeral = bool(kwargs.get("ephemeral"))
    fallback_content = content
    if is_ephemeral:
        mention = interaction.user.mention
        if fallback_content:
            fallback_content = f"{mention} {fallback_content}"
        else:
            fallback_content = mention
        if "allowed_mentions" not in fallback_kwargs:
            fallback_kwargs["allowed_mentions"] = discord.AllowedMentions(
                users=[interaction.user], roles=False, everyone=False
            )

    try:
        await channel.send(fallback_content, **fallback_kwargs)
    ...
    if isinstance(original, discord.HTTPException):
        ...
        if code == 50027:
            await safe_followup_send(
                interaction,
                "Команда отработала слишком долго, поэтому приватный ответ истёк. Результат отправлен сюда.",
                ephemeral=True,
            )
            return
        if code == 40060:
            # Duplicate acknowledge attempt. Suppress noisy tracebacks.
            return
    raise error
...
async def spam(
    ...
    fail_code_counts: dict[int, int] = {}
    anti_spam_stop = False
    anti_spam_stop_after = 0
    ...
    for idx, member in enumerate(targets, start=1):
        try:
            ...
            if code == 50007:
                failed_closed.append(member.display_name[:48])
            else:
                failed_other.append(member.display_name[:48])
            if code == 20026:
                anti_spam_stop = True
                anti_spam_stop_after = idx
                break
        except discord.HTTPException as exc:
            code = getattr(exc, "code", 0) or 0
            if code:
            ...
            if code == 20026:
                ...
                anti_spam_stop_after = idx
                break
        # Anti-spam pacing: short delay every DM and longer cooldown each batch.
        await asyncio.sleep(1.25 + random.uniform(0.35, 0.95))
        if idx % 8 == 0 and idx < len(targets):
            await asyncio.sleep(9.0 + random.uniform(2.0, 5.0))

    lines = [f"Отправлено в ЛС: **{ok}** из {len(targets)}."]
    if failed_closed:
        preview = ", ".join(failed_closed[:20])
        if len(failed_closed) > 20:
            preview += f" … (+{len(failed_closed) - 20})"
        lines.append(f"Не доставлено (ЛС закрыты / бот заблокирован): {preview}")
    if failed_other:
        preview = ", ".join(failed_other[:20])
        if len(failed_other) > 20:
            preview += f" … (+{len(failed_other) - 20})"
        lines.append(f"Не доставлено (прочие ошибки Discord API): {preview}")
    if fail_code_counts:
        top_codes = sorted(
            fail_code_counts.items(), key=lambda kv: kv[1], reverse=True
        )[:4]
        lines.append(
            "Коды ошибок: "
            + ", ".join(f"{code} x{count}" for code, count in top_codes)
        )
    if anti_spam_stop:
        lines.append(
            f"Рассылка остановлена антиспамом Discord (код 20026) после {anti_spam_stop_after} из {len(targets)} получателей."
        )
        print(
            "[spam] "
            f"guild={guild.id} role={role.id} total={len(targets)} ok={ok} "
            + "errors="
